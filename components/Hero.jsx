  ...
  return (
    <section className="hero" aria-label="Intro">
      <div className="hero-grid">
        ...
        >
          <div className="hero-eyebrow">{t.hero.location}</div>
          <h1 className="hero-title" dangerouslySetInnerHTML={{ __html: t.hero.title }} />
          <p className="hero-subtitle">{t.hero.subtitle}</p>
          <div className="hero-cta">
            <a href="#projects" className="btn btn-primary">{t.hero.cta.seeWork}</a>
            <a href="#contact" className="btn btn-secondary">{t.hero.cta.startProject}</a>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="hero-socials" aria-label="Social links">
            <a 
              className="icon-btn" 
              href="https://github.com/phatleework7" 
              target="_blank" 
              rel="noopener noreferrer" 
              aria-label={`${language === 'vi' ? 'GitHub' : 'GitHub'} - ${language === 'vi' ? 'Mở trong tab mới' : 'Opens in new tab'}`}
            >
            ...
            <a 
              className="icon-btn" 
              href="mailto:phatlee.work7@gmail.com" 
              aria-label={language === 'vi' ? 'Gửi Email' : 'Send Email'}
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
